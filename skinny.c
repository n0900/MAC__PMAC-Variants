...
void skinny_64_192_init_tks(skinny_64_192_tweakey_schedule_t *tks, const unsigned char key[24], uint8_t nb_rounds)
{
	...
	/* Load first Tweakey */
	TK[0] = le_load_word16(key);
	TK[1] = le_load_word16(key + 2);
	TK[2] = le_load_word16(key + 4);
	TK[3] = le_load_word16(key + 6);
	...
	/* Load second Tweakey */
	TK[0] = le_load_word16(key + 8);
	TK[1] = le_load_word16(key + 10);
	TK[2] = le_load_word16(key + 12);
	TK[3] = le_load_word16(key + 14);
	...
	/* Load third Tweakey */
	TK[0] = le_load_word16(key + 16);
	TK[1] = le_load_word16(key + 18);
	TK[2] = le_load_word16(key + 20);
	TK[3] = le_load_word16(key + 22);
...
void skinny_64_192_init_keypart(skinny_64_192_tweakey_schedule_t *tks, const unsigned char key[16], uint8_t nb_rounds)
{
	uint16_t TK[4];
	unsigned round;
...
void skinny_64_192_init_tweakpart(skinny_64_192_tweakey_schedule_t *tks, const unsigned char key[8], uint8_t nb_rounds)
{
  uint16_t TK[4];
  unsigned round;
...
void skinny_64_192_rounds_wtks2(skinny_64_192_state_t *state, const skinny_64_192_tweakey_schedule_t *tks1, const skinny_64_192_tweakey_schedule_t *tks2, unsigned first, unsigned last)
{
    uint16_t s0, s1, s2, s3, temp;
    uint8_t rc;
...
void skinny_64_192_encrypt(const unsigned char key[24], unsigned char *output, const unsigned char *input) {
  ...
  /* Unpack the input */
  state.S[0] = le_load_word16(input);
  state.S[1] = le_load_word16(input + 2);
  state.S[2] = le_load_word16(input + 4);
  state.S[3] = le_load_word16(input + 6);

  skinny_64_192_rounds(&state, &tks, 0, SKINNY_64_192_ROUNDS);

  le_store_word16(output,     state.S[0]);
  le_store_word16(output + 2, state.S[1]);
  le_store_word16(output + 4, state.S[2]);
  le_store_word16(output + 6, state.S[3]);
}

void skinny_64_192_encrypt_with_tks(const skinny_64_192_tweakey_schedule_t *tks1, const skinny_64_192_tweakey_schedule_t *tks2, unsigned char *output, const unsigned char *input){
  skinny_64_192_state_t state;
  ...
  skinny_64_192_rounds_wtks2(&state, tks1, tks2, 0, SKINNY_64_192_ROUNDS);
